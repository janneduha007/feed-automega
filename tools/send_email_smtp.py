...
def main():
    ...
    lines.append("   - postupně vyhledej produkty na Carvin.cz podle kódu")
    lines.append("   - u každého nastav: Zakázat objednání")
    lines.append("   - ponechat produkt viditelný (pokud chceš zachovat SEO / URL)")
    lines.append("")
    lines.append("2) NOVÉ PRODUKTY (nově přidané ve feedu - ZOBRAZÍ SE AŽ ZÍTRA!!)")
    lines.append("   - postupně vyhledej produkty na Carvin.cz podle kódu")
    lines.append("   - nastav: Zobrazit produkt")
    lines.append("   - pokud je produkt skrytý, zařaď ho do e-shopu")
    lines.append("   - případně vytvoř / přiřaď kategorii")
    lines.append("")
    lines.append("------------------------------------------------------------------")
